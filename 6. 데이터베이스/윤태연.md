...
특정 열 데이터에 **해시 함수를 적용한 값을 기준**으로 테이블 분할함.
...
> [!TIP]
> 분할된 테이블 조회 시 특정 레코드가 속한 파티션은 파티셔닝 키(partitioning key)를 통해 식별됨. 파티셔닝 키는 테이블의 열을 기준으로 생성됨.

## 샤딩(Sharding)

> [!NOTE]
> 샤딩은 분할된 테이블을 **별개의 데이터베이스 서버에 분산해 저장**하는 기술, **분할 저장된 단위를 샤드**(shard)라고 함

<img width="600" alt="image" src="https://github.com/user-attachments/assets/245bbfa0-acd3-4213-b5fe-e9f8c1113df3" />

수평적 분할 기법의 일종인 샤딩은 일반 파티셔닝과 다른 중요한 특성을 가짐.

1. **서버 분산**: 일반 파티셔닝이 동일 서버 내에서 테이블을 분할하는 반면, 샤딩은 여러 서버에 분산 저장
2. **부하 분산**: 다수의 물리적 서버로 분산되어 데이터베이스 부하를 효과적으로 분산
3. **확장성**: 서버 추가를 통해 수평적 확장이 가능

> [!TIP]
> 샤딩은 대용량 데이터를 다루는 웹 서비스나 분산 데이터베이스 시스템에서 자주 활용됨. NoSQL 데이터베이스 중 많은 시스템이 샤딩을 기본적으로 지원함.
